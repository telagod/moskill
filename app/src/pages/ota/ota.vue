<template>
  <view class="container">
    ...
    <view class="card" v-if="!file">
      <text class="card-title">{{ t('ota.selectFile') }}</text>
      <view class="file-picker" @tap="pickFile">
        <mk-icon name="package" :size="64" color="#e63946"/>
        <text class="picker-text">{{ t('ota.selectFile') }}</text>
    ...
    <view class="card" v-if="status !== 'idle' || progress > 0">
      <text class="card-title">{{ t('ota.uploading') }}</text>
    ...
    <view class="card success-card" v-if="status === 'complete'">
      <mk-icon name="check-circle" :size="80" color="#2ecc71"/>
      <text class="success-title">{{ t('ota.success') }}</text>
      <text class="success-sub">{{ t('ota.success') }}</text>
      <text class="success-hint text-small mt-20">{{ t('ota.warning') }}</text>
    </view>

    <!-- Error State -->
    <view class="card error-card" v-if="status === 'error'">
      <mk-icon name="x-circle" :size="64" color="#e63946"/>
      <text class="error-title">{{ t('ota.failed') }}</text>
      <text class="error-msg">{{ errorMsg }}</text>
      <button class="btn-primary mt-20" @tap="resetOTA">{{ t('common.back') }}</button>
    </view>
    ...
    <view class="ota-actions">
      <button
        ...
        @tap="startUpdate"
      >
        <mk-icon name="lightning" :size="28" color="#fff" :inline="true"/> {{ t('ota.start') }}
      </button>
      ...
      <button
        ...
        @tap="cancelUpdate"
      >
        <mk-icon name="cross" :size="28" color="#e63946" :inline="true"/> {{ t('ota.cancel') }}
      </button>
      ...
      <button
        ...
        @tap="goBack"
      >
        <mk-icon name="back" :size="28" color="#aaa" :inline="true"/> {{ t('common.back') }}
      </button>
    </view>
  </view>
</template>
...
export default {
  setup() {
    ...
    async function startUpdate() {
      ...
      if (!fileData.value) {
        uni.showToast({ title: t('ota.selectFile'), icon: 'none' })
        return
      }
      ...
      progress.value = 0
      statusLog.length = 0
      addLog(t('ota.uploading') + '...', 'info')

      try {
        // Send OTA start command
        addLog(`${t('ota.uploading')} (${formatSize(file.value.size)})`, 'info')
        await otaStart(file.value.size)
        addLog(t('common.success'), 'success')
        ...
        for (let i = 0; i < totalChunks; i++) {
          ...
          if (i % Math.max(1, Math.floor(totalChunks / 10)) === 0) {
            addLog(`${t('ota.uploading')}... ${progress.value.toFixed(0)}%`, 'info')
          }
        }

        addLog(t('ota.verifying'), 'info')
        status.value = 'verifying'

        // Verify CRC
        await otaVerify(fileCRC.value)
        addLog(`CRC32 OK: 0x${fileCRC.value.toString(16).toUpperCase().padStart(8, '0')}`, 'success')

        progress.value = 100
        status.value = 'complete'
        addLog(t('ota.success'), 'success')

      } catch (e) {
        errorMsg.value = e.errMsg || e.message || t('ota.failed')
        status.value = 'error'
        addLog(`${t('common.error')}: ${errorMsg.value}`, 'error')
    ...
    return {
      t, file, fileCRC, status, progress, errorMsg,
