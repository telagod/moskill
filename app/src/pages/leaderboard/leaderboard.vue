  ...
  <view class="container">
    ...
    <view class="podium" v-if="sortedBoard.length >= 3">
      <view class="podium-item silver">
        <view class="podium-medal"><mk-icon name="medal-2" :size="48"/></view>
      ...
      <view class="podium-item gold">
        <view class="podium-medal"><mk-icon name="medal-1" :size="48"/></view>
      ...
      <view class="podium-item bronze">
        <view class="podium-medal"><mk-icon name="medal-3" :size="48"/></view>
      ...
      >
        <view class="board-item-content" :style="{ transform: 'translateX(' + (swipeOffsets[entry.id] || 0) + 'rpx)' }">
          ...
          <view class="board-stats">
            ...
            <text class="board-streak text-small"><mk-icon name="fire" :size="22" color="#f39c12" :inline="true"/>{{ entry.streak }}</text>
          </view>
        </view>
        <!-- Delete button revealed on swipe -->
        <view class="delete-btn" v-if="(swipeOffsets[entry.id] || 0) < -80" @tap="deleteEntry(entry.id)">
          <text class="delete-text">{{ t('leaderboard.delete') }}</text>
        </view>
      </view>

      <view v-if="sortedBoard.length === 0" class="empty-state">
        <view class="empty-icon"><mk-icon name="trophy" :size="80" color="#666"/></view>
        <text class="empty-title">{{ t('leaderboard.empty') }}</text>
        <text class="empty-sub text-small">{{ t('leaderboard.emptyHint') }}</text>
      </view>
    </view>
    ...
    <view class="bottom-action">
      <button class="btn-primary" @tap="saveSession">
        <mk-icon name="save" :size="28" color="#fff" :inline="true"/> {{ t('leaderboard.save') }}
      </button>
    </view>
  </view>
</template>
...
export default {
  setup() {
    ...
    const sortOptions = computed(() => [
      { key: 'totalKills', label: t('leaderboard.sortBy.kills') },
      { key: 'streak', label: t('leaderboard.sortBy.streak') },
      { key: 'efficiency', label: t('leaderboard.sortBy.efficiency') },
      { key: 'killsM', label: t('leaderboard.sortBy.mosquito') },
    ])
    ...
    function deleteEntry(id) {
      ...
      uni.showToast({ title: t('leaderboard.deleted'), icon: 'none' })
    }

    function saveSession() {
      if (!store.state.session) {
        uni.showToast({ title: t('leaderboard.noSession'), icon: 'none' })
        return
      }
      store.dispatch('saveScore')
      uni.showToast({ title: t('leaderboard.saved'), icon: 'success' })
    }
