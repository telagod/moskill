  ...
  <view class="dashboard">
    ...
    <scroll-view scroll-y class="dash-scroll">
      <view class="container">
        ...
        <view class="card">
          ...
          <view class="class-grid">
            <view class="class-item" v-for="(cls, idx) in classifications" :key="cls.label">
              <text class="class-icon">{{ cls.icon }}</text>
        ...
        <!-- Environment -->
        <view class="card">
          <text class="card-title">Environment</text>
          <view class="env-row flex-between">
            <view class="env-item">
              <text class="env-icon">🌡️</text>
              <text class="env-value">{{ env.temp }}°C</text>
              <text class="env-label text-small">Temperature</text>
            </view>
            <view class="env-item">
              <text class="env-icon">💧</text>
              <text class="env-value">{{ env.humi }}%</text>
              <text class="env-label text-small">Humidity</text>
            </view>
            <view class="env-item">
              <text class="env-icon">⏱️</text>
              <text class="env-value">{{ sessionTime }}</text>
              <text class="env-label text-small">Duration</text>
            </view>
          </view>
        </view>
        ...
        <!-- Quick Actions -->
        <view class="card">
          <text class="card-title">Quick Actions</text>
          <view class="action-grid">
            <view class="action-btn" @tap="toggleHV">
              <text class="action-icon">⚡</text>
              <text class="action-label">{{ hvOn ? 'HV ON' : 'HV OFF' }}</text>
            </view>
            <view class="action-btn" @tap="goStats">
              <text class="action-icon">📊</text>
              <text class="action-label">Stats</text>
            </view>
            <view class="action-btn" @tap="goControl">
              <text class="action-icon">⚙️</text>
              <text class="action-label">Control</text>
            </view>
          </view>
        </view>
        ...
        <!-- Kill Feed -->
        <view class="card">
          <text class="card-title">Live Kill Feed</text>
          <view class="kill-feed">
            ...
            <view v-if="killFeed.length === 0" class="feed-empty">
              <text class="text-small">Waiting for kills...</text>
            </view>
          </view>
        </view>
      </view>
...
export default {
  setup() {
    ...
    const classifications = computed(() => {
      ...
      if (!s) return [
        { icon: '🪰', label: 'S', count: 0 },
        { icon: '🦟', label: 'M', count: 0 },
        { icon: '🪳', label: 'L', count: 0 },
        { icon: '🦋', label: 'XL', count: 0 },
      ]
      return [
        { icon: '🪰', label: 'S', count: s.killsS },
        { icon: '🦟', label: 'M', count: s.killsM },
        { icon: '🪳', label: 'L', count: s.killsL },
        { icon: '🦋', label: 'XL', count: s.killsXL },
      ]
    })
    ...
    return {
      ...
      hvOn, toggleHV, goStats, goControl, formatTime
    }
  }
}
</script>
